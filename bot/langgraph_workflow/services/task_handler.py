...
def handle_task_command(query, user_name="", user_id="",
    ...
    if any(kw in text for kw in ["临近", "最近", "快到期", "即将"]):
        ...
        for t in upcoming:
            ...
            lines.append(f"  #{t.id} {t.trigger_time} {who} {t.content}")
        return "\n".join(lines)

    # ===== 查看任务 =====
    if any(kw in text for kw in [
        "查看任务", "我的任务", "任务列表", "任务有哪些",
        "什么任务", "任务是什么", "还有任务", "任务情况",
    ]):
        tasks = scheduler.list_tasks(user_id=user_id)
        if not tasks:
            return "当前没有待办任务。"
    ...
    scheduler.add_task(
        content=task_content, trigger_time=trigger_time,
        creator_name=user_name, creator_id=user_id,
        assignee_name=assignee_name or user_name, assignee_id=user_id,
        schedule_type=schedule_type, schedule_weekdays=schedule_weekdays,
    )
