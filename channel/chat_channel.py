...
class ChatChannel(Channel):
    ...
    def _compose_context(self, ctype: ContextType, content, **kwargs):
        ...
        if ctype == ContextType.TEXT:
            ...
            # if context.get("isgroup", False):
            #     logger.warning("[chat_channel] File message in group chat is not supported by DingTalk")
            #     return None
            
            nick_name = cmsg.from_user_nickname
            nick_name_black_list = conf().get("nick_name_black_list", [])
            if nick_name and nick_name in nick_name_black_list:
                logger.warning(f"[chat_channel] Nickname '{nick_name}' in BlackList, ignore file")
                return None
            
            file_info = cmsg.file_content if hasattr(cmsg, 'file_content') else None
            if not file_info:
                logger.warning("[chat_channel] File message has no file_content")
                return None
            
            file_name = file_info.get('file_name', '')            
            logger.info(f"[chat_channel] Receive file: name={file_name}")
            context["file_info"] = file_info
            context["file_path"] = content
            context["file_name"] = file_name
        elif context.type == ContextType.IMAGE:
            logger.debug("[DingTalk]receive image msg: {}".format(content))
            context.content = content
        elif context.type == ContextType.DAILY_NEWS:
            logger.debug("[DingTalk]receive daily news msg: {}".format(content))
            context.content = content
    ...
    def _generate_reply(self, context: Context, reply: Reply = Reply()) -> Reply:
        ...
        if not e_context.is_pass():
            ...
            if context.type == ContextType.TEXT or context.type == ContextType.IMAGE_CREATE or context.type == ContextType.FILE or context.type == ContextType.DAILY_NEWS or context.type == ContextType.IMAGE:  # 文字和图片消息 新增文件、图片处理
